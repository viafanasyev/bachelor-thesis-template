...
Для листингов используйте пакет `listings` (уже подключён в классе). Основной стиль уже установлен по умолчанию, так что вам нужно лишь выбрать язык, сделать подпись и добавить метку.
...
Кроме всех поддерживаемых пакетом `listings` языков, также можно использовать язык Kotlin (стиль взят [отсюда](https://github.com/cansik/kotlin-latex-listing)).
...
### У меня есть соруководитель, нужно и можно ли его как-то указать?

В [шаблоне](https://www.hse.ru/ba/se/assessment) титульного листа, взятого с сайта НИУ ВШЭ, я не нашёл места, где можно было бы указать соруководителя. Моё предположение - соруководитель не указывается на титульном листе. Если есть другие идеи - прошу создавать [issue](https://github.com/viafanasyev/bachelor-thesis-template/issues).
